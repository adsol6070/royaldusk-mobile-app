...
class MySavedController extends GetxController {
  ...
  RxList<Package> savedPackages = <Package>[].obs;
    ...
    try {
      ...
      if (savedData != null && savedData.isNotEmpty) {
        ...
        for (var packageData in savedData) {
          try {
            savedPackages
                .add(Package.fromJson(packageData as Map<String, dynamic>));
          } catch (e) {
  ...
  Map<String, dynamic> _packageToJson(Package package) {
    return {
      'id': package.id,
      'name': package.name,
      'slug': package.slug,
  ...
  // Toggle save status
  Future<bool> togglePackageSave(Package package) async {
    if (isPackageSaved(package.id)) {
  ...
  List<Package> get myTripList => savedPackages;
