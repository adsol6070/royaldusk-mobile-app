import 'package:royaldusk_mobile_app/app/ui/place/popular_place_view.dart';
...
class PopularPlaceScreen extends StatefulWidget {
  const PopularPlaceScreen({Key? key}) : super(key: key);

  @override
  PopularPlaceScreenState createState() => PopularPlaceScreenState();
}

class PopularPlaceScreenState extends State<PopularPlaceScreen> {
  late PopularPlacesController controller;
  var totalRecords = 30;
  ...
  void initState() {
    super.initState();
    controller = PopularPlacesController();
    isDarkMode = controller.themeController.isDarkMode;
  }

  @override
  Widget build(BuildContext context) {
    return GetBuilder<PopularPlacesController>(
        init: controller,
        tag: 'travel_popular_places',
        // theme: theme,
        builder: (controller) {
          return Scaffold(
            appBar: commonAppBarWidget(context, titleText: popularPlaces),
            body: SafeArea(
              child: Padding(
    ...
    await showMenu(
      ...
      items: [
        ...
        PopupMenuItem(
          ...
          child: Obx(() => RadioListTile(
                activeColor: appColorPrimary,
                title: const Text(highestReview),
                value: '3',
                groupValue: controller.selectedOption.value,
                onChanged: (value) {
                  controller.setSelectedOption(value!);
